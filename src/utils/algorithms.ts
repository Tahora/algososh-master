...
import {cleanItems, setItems} from "../services/actions/items";
import {smallTimeout, timeout, timer} from "./constats";
                         ...
                         changes:
                             {
                                 ...
                                 head?:number,
                                 tail?:number},
                                pause:number=timeout)
{
    changes.changingIndx?.forEach((i)=>{array[i].state=ElementStates.Changing})
    changes.modifiedIndx?.forEach((i)=>{array[i].state=ElementStates.Modified})
    changes.defaultIndx?.forEach((i)=>{array[i].state=ElementStates.Default})
    ...
    if(dispatch(setItems(array)) ) {return true}
...
export async function stringExpand(text:string,dispatch:TAppDispatch )
{
    ...
    const textArray=Array.from(text).map((i)=>{const x:CircleProps={letter:`${i}`}; return x; });
...
export async function bubbleSort(arr:ColumnProps[],direction:Direction=Direction.Ascending ,dispatch:TAppDispatch )
{
    ...
    for (let i = 0; i < arr.length; i++) {
        for (let j = 0; j < arr.length - i - 1; j++) {
         ...
         if(await changeArr(arr,dispatch,{modifiedIndx:i==arr.length-2?[arr.length - i-1,0]:[arr.length - i-1], defaultIndx:i<arr.length-2?[arr.length - i-2]:[]}))
         {return}
    }
}
...
export async function selectionSort(arr:ColumnProps[],direction:Direction=Direction.Ascending ,dispatch:TAppDispatch ) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        for (let j = i+1; j < arr.length ; j++) {
            if(isSwap(arr[minIndx], arr[j])) {minIndx=j}
            if(await changeArr(arr,dispatch,{changingIndx:[j], defaultIndx:j>i+1?[j-1]:[]}))
            {return}
        }
        swap(arr,i, minIndx)
        if(await changeArr(arr,dispatch,{modifiedIndx:[i],defaultIndx:i<arr.length-1?[arr.length-1]:[] }))
        {return}
